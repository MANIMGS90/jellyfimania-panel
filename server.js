...
app.post("/api/panel-users", authMiddleware, (req, res) => {
  ...
  res.json({ id: user.id, username: user.username, role: user.role, password });
});
...
// Eliminar un seller/reseller (ej. por mal uso del panel). Solo quien
// puede administrarlo (su padre o el superadmin) puede borrarlo, y
// no se permite borrar si todavia tiene sub-usuarios o cuentas de
// clientes activas a su nombre, para no dejar cosas huerfanas.
app.delete("/api/panel-users/:id", authMiddleware, (req, res) => {
  const target = db.findUserById(req.params.id);
  if (!target) return res.status(404).json({ error: "No encontrado" });
  if (!canManage(req.user, target.id) || target.id === req.user.id) {
    return res.status(403).json({ error: "Sin permiso para eliminar este usuario" });
  }

  const hasChildren = db.allUsers().some((u) => u.parent_id === target.id);
  if (hasChildren) {
    return res.status(400).json({ error: "Este usuario tiene sub-usuarios (resellers) a su cargo. Eliminalos primero." });
  }
  const hasAccounts = db.allMediaAccounts().some((a) => a.owner_user_id === target.id && a.status !== "deleted");
  if (hasAccounts) {
    return res.status(400).json({ error: "Este usuario todavia tiene cuentas de clientes activas. Reasignalas o eliminalas primero." });
  }
...
// Quitar creditos que se le dieron de mas a un seller/reseller por error.
// Es el espejo de /credits/transfer: le resta al usuario destino y se
// los regresa a quien los quita (si no es cuenta infinita).
app.post("/api/credits/deduct", authMiddleware, (req, res) => {
  const { user_id, amount } = req.body || {};
  const amt = parseInt(amount, 10);
  if (!user_id || !amt || amt <= 0) return res.status(400).json({ error: "Datos invalidos" });

  if (!canManage(req.user, user_id)) {
    return res.status(403).json({ error: "No puedes quitar creditos a ese usuario" });
  }

  const target = db.findUserById ? db.findUserById(user_id) : null;
  const targetCredits = target ? target.credits : null;
  if (target && !target.is_infinite && targetCredits < amt) {
    return res.status(400).json({ error: "Ese usuario no tiene tantos creditos para quitar" });
  }

  if (target && !target.is_infinite) {
    db.updateUserCredits(user_id, -amt);
  }
  if (!req.user.is_infinite) {
    db.updateUserCredits(req.user.id, amt);
  }
  db.insertCreditMove({ from_user_id: Number(user_id), to_user_id: req.user.id, amount: amt, reason: "correccion" });
...
// Suspender/reactivar manualmente, sin borrar la cuenta (ej. cliente
// se atraso en el pago). Distinto de la suspension automatica por
// vencimiento: esta la dispara el admin/seller a mano en cualquier
// momento, y tambien puede revertirla (reactivar) cuando quiera.
app.post("/api/accounts/:id/status", authMiddleware, async (req, res) => {
  ...
  const disable = action === "suspend";

  try {
    if ((acc.service === "jellyfin" || acc.service === "emby") && acc.service_user_id) {
      const cfg = db.getSetting(acc.service, {});
      if (cfg.baseUrl && cfg.apiKey) {
        await mediaServer.setUserDisabled(cfg.baseUrl, cfg.apiKey, acc.service_user_id, disable);
      }
    } else if (acc.service === "plex") {
      return res.status(400).json({ error: "Plex no soporta suspender: solo eliminar el acceso compartido." });
    }
    db.updateMediaAccountStatus(acc.id, disable ? "suspended" : "active");
    res.json({ ok: true, status: disable ? "suspended" : "active" });
  } catch (e) {
    res.status(400).json({ error: e.message });
  }
});
