...
async function loadAccounts() {
  ...
  for (const acc of rows) {
    ...
    const typeBadge = acc.is_demo
      ...
      : `Plan`;
    const devicesLabel = acc.max_devices
      ? `${acc.max_devices} ${acc.max_devices == 1 ? "dispositivo" : "dispositivos"}`
      : "-";
    ...
    tbody.innerHTML += `<tr>
      ...
      <td>${new Date(acc.expires_at).toLocaleString()}</td>
      <td>
        ${acc.service === "plex" ? "" : acc.status === "active"
          ? `<button class="btn btn-secondary" data-toggle-account="${acc.id}" data-action="suspend">Suspender</button>`
          : `<button class="btn btn-secondary" data-toggle-account="${acc.id}" data-action="activate">Reactivar</button>`
        }
        <button class="btn btn-danger" data-del-account="${acc.id}">Eliminar</button>
      </td>
    </tr>`;
  }

  tbody.querySelectorAll("[data-toggle-account]").forEach((btn) => {
    btn.addEventListener("click", async () => {
      const action = btn.dataset.action;
      const label = action === "suspend" ? "suspender" : "reactivar";
      if (!confirm(`¿Seguro que quieres ${label} esta cuenta?`)) return;
      try {
        await api("/accounts/" + btn.dataset.toggleAccount + "/status", {
          method: "POST",
          body: { action },
        });
        loadAccounts();
...
$("#formNewAccount").addEventListener("submit", async (e) => {
  ...
  const body = {
    ...
    plan: $("#accPlan").value,
    max_devices: parseInt($("#accMaxDevices").value, 10) || 1,
...
async function loadPanelUsers() {
  ...
  for (const u of rows) {
    tbody.innerHTML += `<tr>
      ...
      <td>
        <button class="btn btn-secondary" data-credits="${u.id}" data-name="${u.username}">Dar créditos</button>
        <button class="btn btn-secondary" data-deduct="${u.id}" data-name="${u.username}">Quitar créditos</button>
        <button class="btn btn-danger" data-del-user="${u.id}" data-name="${u.username}">Eliminar</button>
      </td>
    </tr>`;
  }
  tbody.querySelectorAll("[data-credits]").forEach((btn) => {
    btn.addEventListener("click", () => openCreditsModal(btn.dataset.credits, btn.dataset.name, "give"));
  });
  tbody.querySelectorAll("[data-deduct]").forEach((btn) => {
    btn.addEventListener("click", () => openCreditsModal(btn.dataset.deduct, btn.dataset.name, "remove"));
  });
  tbody.querySelectorAll("[data-del-user]").forEach((btn) => {
    btn.addEventListener("click", async () => {
      if (!confirm(`¿Eliminar a "${btn.dataset.name}"? Esto no se puede deshacer.`)) return;
      try {
        await api("/panel-users/" + btn.dataset.delUser, { method: "DELETE" });
        loadPanelUsers();
      } catch (err) {
        alert(err.message);
      }
...
$("#formNewPanelUser").addEventListener("submit", async (e) => {
  ...
  try {
    const created = await api("/panel-users", { method: "POST", body });
    $("#modalNewPanelUser").classList.add("hidden");
    $("#formNewPanelUser").reset();
    loadPanelUsers();
    alert(
      `Usuario creado con éxito.\n\nUsuario: ${created.username}\nContraseña: ${created.password}\n\n` +
      `Guárdala en un lugar seguro: por seguridad, el panel no la vuelve a mostrar después de este momento ` +
      `(si la pierde, puedes eliminar el usuario y crear uno nuevo).`
    );
  } catch (err) {
...
let creditsMode = "give";
function openCreditsModal(userId, username, mode) {
  creditsTargetId = userId;
  creditsMode = mode || "give";
  $("#creditsTargetLabel").textContent = (creditsMode === "give" ? "Para: " : "A quitarle a: ") + username;
  $("#modalCredits h2").textContent = creditsMode === "give" ? "Dar créditos" : "Quitar créditos";
  $("#formCredits button[type=submit]").textContent = creditsMode === "give" ? "Enviar" : "Quitar";
...
$("#formCredits").addEventListener("submit", async (e) => {
  ...
  try {
    if (creditsMode === "give") {
      ...
      await api("/credits/deduct", {
        method: "POST",
        body: { user_id: parseInt(creditsTargetId, 10), amount: parseInt($("#creditsAmount").value, 10) },
      });
    }
